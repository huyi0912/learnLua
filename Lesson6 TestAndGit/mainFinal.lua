...
function love.keypressed(key, scancode)
    ...
    elseif key == "right" then                        --按下右鍵
        ball.body:applyForce(speed, 0)
    elseif key == "space" or scancode == "space" then -- 按下空白鍵，重置小球位置和速度
        -- 1. 將小球重置 Y 軸位置回初始位置 (Y: 100)
        ball.body:setPosition(ball.body:getX(), 100)  -- 只重置 Y 軸位置，保持 X 軸位置不變

        -- 2. 將小球的移動速度歸零 (X方向速度: 0, Y方向速度: 0)
